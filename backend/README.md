...
管理员可在 MCP Server 的请求头中配置签名用户上下文头：把某个请求头的值填为占位符 `${DEEIX_SIGNED_USER_CONTEXT}`，每次 `tools/call` 时该头会被替换为 HMAC-SHA256 签名的 token（payload 含 `user_id`、`conversation_id`、`request_id` 与过期时间，签名密钥复用 `JWT_SECRET`，默认有效期 5 分钟）。MCP 服务端可用同一密钥校验，供外部网关按用户隔离单租户 MCP 工具。未配置占位符的服务端不会收到任何额外请求头。
